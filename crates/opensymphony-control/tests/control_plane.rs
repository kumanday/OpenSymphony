...
async fn stream_updates_times_out_when_event_stream_opens_without_bootstrap_snapshot() {
    ...
    let server_task = tokio::spawn(async move {
        let (mut connection, _) = listener
            .accept()
            .await
            .expect("client should connect to the SSE listener");
        connection
            .write_all(
                b"HTTP/1.1 200 OK\r\ncontent-type: text/event-stream\r\ncache-control: no-cache\r\nconnection: keep-alive\r\n\r\n",
            )
    ...
    let error = stream
        .next()
        .await
        .expect("missing bootstrap snapshot should surface a timeout error")
        .expect_err("headers-only stream should not decode as a snapshot");
