...
- hosted identity, auth, and RBAC hardening coverage in `opensymphony-gateway/tests/auth_rbac.rs`: login/logout/session probe, HTTP read allowed and denied (unauthenticated 401, restricted-viewer project-access 403 with `permission_denied`/`forbidden_resource` code, permitted project passes), run read viewer floor, action dispatch allowed for member (carries actor context) and denied for viewer (receipt includes rejection reason + denial code), WebSocket upgrade denied without token and accepted with `?token=`, dev bypass refused in production and granted in dev mode, and disabled mode preserving the unauthenticated local contract; plus RBAC unit tests in `opensymphony-gateway/src/rbac.rs` for read floor per resource, project-access rules, action operate/admin requirements, planning-session admin requirement, and dev bypass
