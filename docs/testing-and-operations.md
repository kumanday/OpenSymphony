...
- `crates/opensymphony-cli/tests/linear_mcp.rs` drives the real `opensymphony linear-mcp --stdio` child process through MCP initialization, tool listing, and comment/transition/link/state-list calls against a local fake Linear GraphQL server
