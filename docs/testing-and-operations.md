...
- `cargo test --test workspace_manager` covers durable ownership-marker sweeps, preservation of foreign generation-shaped directories, receipt-owned recovery, and the retry verification mode that permits legitimate worker changes while retaining checkout provenance checks
