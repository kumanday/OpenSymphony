...
- `opensymphony-orchestrator`
  ...
  - fixed continuation retry, exponential failure backoff, stall detection, retry-start validation for `due_at`, `attempt`, and bounded global/per-state capacity, reconciliation of running, retry-queued, and claimed-only issues including a claim-to-start grace window before stale claimed-only release, and restart recovery
