...
The persisted conversation manifest also records the owning issue reference, active reuse policy, creation and attach timestamps, the launch profile used to create the thread, and a hashed LLM configuration fingerprint so `opensymphony debug <issue-id>` can reattach to the same session or rehydrate the same `conversation_id` with matching runtime settings without persisting provider secrets in plaintext. Persisting the policy keeps recovery deterministic when the workflow later changes from one policy to another.
Before reusing a persisted conversation, the runner now compares the current workflow-resolved LLM model plus hashed `api_key` and `base_url` values against the manifest or live conversation summary. If those settings drift, OpenSymphony deletes and recreates the same OpenHands `conversation_id`, then seeds the new fresh thread with the full workflow prompt plus Linear workpad recovery context because the old OpenHands event history is no longer present.
...
workflow-owned WebSocket readiness and reconnect budgets. The runtime also now
accepts workflow-owned `openhands.local_server.command` overrides for managed
local supervision and resolves workflow-owned `agent.llm.api_key_env` and
`agent.llm.base_url_env` names when building the conversation-create payload.
The typed conversation-create request now forwards workflow-owned
`openhands.mcp.stdio_servers` entries through `mcp_config.stdio_servers`.
Explicit `local_server.enabled`, `local_server.env`,
