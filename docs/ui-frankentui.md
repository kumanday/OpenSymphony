...
- if `/api/v1/events` never finishes connecting or opening, fail that attach attempt within the bounded stream-open timeout and retry instead of waiting forever in `conn=connecting` or `conn=reconnecting`
