...
- `RuntimeStreamConfig::replay_existing_events_on_attach` lets diagnostic clients request the pinned server's `resend_all=true` WebSocket replay when attaching to a restored persisted conversation that may not have an active in-memory state snapshot; normal orchestrator runtime streams leave this disabled and continue to rely on the live readiness frame plus REST reconciliation
...
- runtime attach is reached only after the orchestrator has verified the terminal checkout envelope; reconnect and replay preserve that conversation binding rather than selecting a workspace from mutable current-directory configuration
