...
- workflow resolution now forwards `openhands.conversation.agent.tools` and optional `openhands.conversation.agent.include_default_tools` into the typed OpenHands conversation-create payload; partial agent overrides still inherit the default LLM model unless the workflow explicitly replaces it, and when the tool fields are omitted OpenSymphony adds `terminal` and `file_editor` as the default coding-agent tools while leaving the agent-server default `FinishTool` and `ThinkTool` set under OpenHands control
