...
- the current orchestrator/runtime path consumes workflow-owned `reuse_policy` values: `per_issue` reuses the persisted manifest, `fresh_each_run` always creates a new thread, and policy drift or unsupported values are surfaced explicitly instead of being silently ignored
