...
- Explicit workflow-owned `openhands.local_server.command` overrides are supported only for daemon-managed local supervision; external, authenticated, path-prefixed, or `local_server.enabled: false` targets must fail deterministically at the runtime boundary instead of silently ignoring the override.
